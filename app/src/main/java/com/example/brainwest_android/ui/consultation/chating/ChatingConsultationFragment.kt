...
class ChatingConsultationFragment : Fragment() {
    ...
    private val viewModel: ChatingConsultationViewModel by viewModels {
        ChatingConsultationViewModelFactory(ConsultationRepository(), ChatConsultationRepository(requireContext()))
    }
    ...
    override fun onCreateView(
        ...
        doctorId = requireArguments().getInt("doctor_user_id")
    ...
    fun sendMessage() {
        viewModel.sendMessageToAPI(doctorId, binding.etMessage.text.toString())
        ...
        viewModel.getByIdResult.observe(viewLifecycleOwner) {state ->
            when (state) {
                ...
                is State.Success -> {
                    ...
                    binding.tvRating.text = state.data.rating.toString()
                    binding.tvExperiences.text = "154 Ulasan, Pengalaman ${state.data.experience} Tahun"
